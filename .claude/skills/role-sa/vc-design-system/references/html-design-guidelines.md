...
Claude's built-in image generation capabilities can be used for hero banners, conceptual illustrations, and decorative accents that establish the page's visual tone.
...
For `--slides` presentation-grade output, consider invoking the `frontend-design` skill for richer style selection and distinctive font/palette pairing.
