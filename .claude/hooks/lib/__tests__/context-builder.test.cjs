#!/usr/bin/env node
/**
 * Tests for context-builder.cjs - protocol/workflows resolution compatibility
 ...
 * - ~/.claude/rules/
 * Key scenarios:
 * - resolveRulesPath() checks canonical protocol files first
 * - Falls back to workflows/ if canonical file not found
  ...
  describe('CLAUDE.md reference resolution', () => {
    ...
    it('resolves canonical protocol filenames and legacy aliases correctly', () => {
      ...
      for (const [file, expectedPath] of Object.entries(expected)) {
        ...
        assert.strictEqual(result, expectedPath, `Should resolve ${file}`);
      }
    });
