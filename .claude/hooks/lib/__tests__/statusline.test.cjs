      ...
      content: [
        {
          ...
          id: 'agent-1',
          name: 'Task',
          input: { subagent_type: 'research-agent', model: 'claude-opus', description: 'Research topic' }
        }
      ]
    }
  },
...
test('parseTranscript tracks agents correctly', async () => {
  ...
  assertEquals(agent.type, 'research-agent', 'Should capture agent type');
...
test('getGitInfo timeout does not cache null result', () => {
  ...
  const cachePath = path.join(os.tmpdir(), `vc-git-cache-${hash}.json`);
